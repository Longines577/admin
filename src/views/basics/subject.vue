	...
	<div class="table">
		...
		<!--工具条-->
				<el-form :inline="true" :model="filters" >
					...
					<el-form-item label="科目">
						<el-input v-model="filters.code" placeholder="请输入科目" size="small" clearable filterable></el-input>
					</el-form-item>
		...
		<!--列表-->
			<el-table :data="users" border highlight-current-row stripe v-loading="loading" style="width: 100%;">
				...
				<el-table-column  align="center" prop="firstName" label="一级项目">
				</el-table-column>
				<el-table-column  align="center" prop="secondName" label="二级项目" >
				</el-table-column>
		...
		<el-dialog :close-on-click-modal='false' :title=" isEdit ? '编辑' : '添加'" :visible.sync="isShowEdit" center>
			<el-form :model="editForm" label-width="80px" ref="editForm">
				<el-form-item label="一级项目" prop="firstId" :rules="[{ required: true, message: '请填写一级项目', trigger: 'blur' }]">
					<el-select v-model="editForm.firstId" :disabled="isEdit==1?true:false"  placeholder="请选择" clearable filterable @change="getoption2">
						<el-option v-for="item in option1" :key="item.value" :label="item.firstName" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="二级项目" v-if='isEdit!=1' prop="secondId" :rules="[{ required: true, message: '请填写二级项目', trigger: 'blur' }]" >
					<el-select v-model="editForm.secondId" placeholder="请选择" clearable filterable>
						<el-option v-for="item in option2" :key="item.value" :label="item.secondName" :value="item.id"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="二级项目"  v-if='isEdit==1' prop="secondName" :rules="[{ required: true, message: '请填写二级项目', trigger: 'blur' }]" >
					<el-select v-model="editForm.secondName" disabled placeholder="请选择" clearable filterable> 
						<el-option v-for="item in option2" :key="item.value" :label="item.secondName" :value="item.secondName"></el-option>
					</el-select>
				</el-form-item>
				<el-form-item label="科目名称" prop="itemId" :rules="[{ required: true, message: '请填写科目名称', trigger: 'blur' }]">
					<el-select v-model="editForm.itemId" placeholder="请选择" clearable filterable>
...
export default {
	...
	data() {
		return {
			...
			apiName: 'system/api/subject/listSubject',
			addName: 'system/api/subject/addSubject',
			updateName: 'system/api/subject/updateSubject',
			delName:'system/api/subject/delSubject',
			editForm: {},
			option1: [],
			option2: [],
			option3:[],
