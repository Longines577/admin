...
import{updateTeacherInfoByHt} from '../../../api/api'
export default {
  mixins: [listMixin],
  data() {
    return {
      ...
      upDateTeacherInfoByHtName:'system/api/courseTurnsTime/updateTeacherInfoByHt',
      option5: [],
      option6: [],
      option7: [],
      dataList:null,
  ...
  methods: {
    saveTransaction(){
       let params = {
        ...
        liveId:this.dataList.liveId
      };
      updateTeacherInfoByHt(params).then(res=>{
          if(res.code === 30000){
            console.log(res,33)
          }
      }).catch(error=>{
         this.$message.error(this.upDateTeacherInfoByHtName + "接口出错！");
      })
      console.log('我是同步')
    },
  },
